...
public class CacheFacade implements InitializingBean {
	...
	public YtmWordListEntity getAWord() throws Exception {
		...
		try {
			...
			Query query = cache.createQuery().addCriteria(cache.getSearchAttribute("isComplete").eq(Boolean.FALSE).and(cache.getSearchAttribute("isWorking").eq(Boolean.FALSE)).or(cache.getSearchAttribute("isWorking").eq(Boolean.TRUE).and(cache.getSearchAttribute("lastDate").le(now))));
			query.maxResults(1);
			query.includeValues();
